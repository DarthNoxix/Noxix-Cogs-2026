...
log = logging.getLogger("red.openwebui.models")

_chroma_client = chromadb.Client()


class OpenWebUIBaseModel(BaseModel):
    @classmethod
...
class Embedding(OpenWebUIBaseModel):
    text: str
    ...
    model: str = "bge-large-en-v1.5"  # Default OpenWebUI embedding model
...
class CustomFunction(OpenWebUIBaseModel):
...
class Usage(OpenWebUIBaseModel):
    total_tokens: int = 0
    input_tokens: int = 0
    output_tokens: int = 0


class GuildSettings(OpenWebUIBaseModel):
    ...
    question_mode: bool = False  # If True, only the first message and messages that end with ? will have embeddings
    ...
    api_base: t.Optional[str] = None  # OpenWebUI API base URL
    ...
    model: str = "deepseek-r1:8b"  # Default OpenWebUI model
    embed_model: str = "bge-large-en-v1.5"  # Default OpenWebUI embedding model
    ...
    auto_answer_model: str = "deepseek-r1:8b"  # Model to use for auto-answer
    ...
    def sync_embeddings(self, guild_id: int):
        try:
            collection = _chroma_client.get_collection(f"openwebui-{guild_id}")
...
class Conversation(OpenWebUIBaseModel):
...
class NoAPIKey(Exception):
    """OpenWebUI API Key not set"""


class EmbeddingEntryExists(Exception):
    """Entry name for embedding exists"""
